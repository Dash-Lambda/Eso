...
Currently, compiled BrainFuck programs use a blocking queue to build the output. The program runs until it produces output, which it writes to the queue and doesn't proceed until Eso reads that output from the queue.
