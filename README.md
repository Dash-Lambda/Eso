...
* Unispace (difficulty with finding particularly good documentation)
* Funciton (difficulty handling lazy input with its reversed string encoding)
* Wierd
...
The Scala 'interpreter' compiles the source code to an abstract syntax tree and directly executes that. It treats code similarly to the REPL.

Its original purpose is to run programs generated by Eso's transpilers. Currently, the Scala 'interpreter' and the internal compiling interpreters are different; The internal compiling interpreters cast the code to a specific class, while the Scala 'interpreter' makes no assumptions. This ultimately means the internal ones are faster but the Scala interpreter is more versatile. 
