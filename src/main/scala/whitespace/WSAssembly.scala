...
import common.{TranslatorException, Translator}
...
object WSAssembly extends Translator {
  ...
  def apply(bools: mutable.HashMap[String, (Boolean, String)], nums: mutable.HashMap[String, (Int, String)])(prog: String): Try[String] = {
    getParms(bools, nums)("debug")() match{
      ...
      case None => Failure(TranslatorException("Missing Configuration Values"))
    }
  }
        ...
        op match{
          ...
          case tok => argOps.find(tok.startsWith) match{
            case Some(key) =>
              ...
              else Failure(TranslatorException("Not Enough Arguments"))
            case None => Failure(TranslatorException("Operation Not Recognized"))
          }
        }
      case _ => Success(ac)
    }
