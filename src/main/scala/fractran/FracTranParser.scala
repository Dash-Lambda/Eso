...
import common.{EsoExcep, EsoObj, EsoParser, PrimeNumTools, RegexParser}
...
object FracTranParser extends EsoObj{
  ...
  lazy val fracParser: EsoParser[String, (SafeLong, SafeLong)] = RegexParser[(SafeLong, SafeLong)](fracReg){m => (toNum(m.group(1)), toNum(m.group(2)))}
  ...
  def parse(progRaw: String): Try[(SafeLong, Vector[(SafeLong, SafeLong)])] = {
    initReg.findFirstIn(progRaw) match{
      case Some(init) => Success((toNum(init), fracParser.parseAll(progRaw)))
